...
<html lang="en">
    ...
    <body>
        ...
        <%         if ((!header.endsWith("login.jsp")) || (header.endsWith("login.jsp") && (check_mail2.startsWith("0")||pass.compareTo(request.getParameter("pwd")) != 0) && request.getParameter("fromlog").startsWith("yeslog")) || (header.endsWith("login.jsp") && (!check_mail2.startsWith("0")) && (!request.getParameter("fromlog").startsWith("yeslog")))) {
                if (header.endsWith("login.jsp") && ( request.getParameter("fromlog").startsWith("yeslog"))) {
                   
        %>
        <div style="margin-left: 500px"><br><br><br><br><br>Wrong email/password. Try Again</div>

        <%} else if ((header.endsWith("login.jsp") && (!check_mail2.startsWith("0")) && (!request.getParameter("fromlog").startsWith("yeslog")))) {%>
        ...
        if(check_mail2.startsWith("0")) {   
                ...
                stmt2.executeQuery("insert into customer_product values ("+now_id+",0,'ThisIsDummyProduct',100,90,10-10-2016,0,10,2,'/Kg')");
                //out.println("insert into customer_product values ("+now_id+",0,'ThisIsDummyProduct',100,90,10-10-2016,0,10,2,'/Kg')");
            }
            session.setAttribute("userid", request.getParameter("email"));%>    
        
        <%if(session.getAttribute("userid").toString().length()>4){
            //session.setAttribute("userid", null);
